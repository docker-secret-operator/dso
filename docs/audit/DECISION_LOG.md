...
**Fixed in this pass (verified: `go build`/`go vet`/`gofmt` clean, `golangci-lint` 0 issues, `gosec` 0 issues, `go test -race ./...` all packages `ok`):**

- **`internal/strategy/decision_engine.go`** — `IsStateful` was a -20 score penalty, not a hard rule, so a stateful container with a health check and no fixed port (`100 - 20 = 80 >= 70`) could still be assigned the "rolling" (parallel-instance) rotation strategy — running two live copies of a database concurrently, the exact data-corruption risk the code's own comments warn about. Changed to a hard override: `IsStateful` now forces `"restart"` regardless of score. The existing `TestDecideStrategy/Stateful_application` subtest had encoded the buggy behavior as its expectation (`expectedStrategy: "rolling"`) — corrected to `"restart"` with the reasoning inline, rather than leaving a test that would silently re-permit the bug if anyone "simplified" the override away.
- **`internal/setup/doctor_permissions.go`** (`checkSocketPermissions`, DSO-DOCTOR-004) — tested `mode&0002` (world-*writable*) while every part of the check's identity (id, name, description, warning message) claims to test world-*readable*. The correctly-implemented sibling check (`checkConfigPermissions`, `mode&0004`) two functions below shows what it should have been. A world-readable-only Docker socket (`0644`) — letting any local user read Docker API traffic and, on many setups, connect to the daemon — passed this check silently. Fixed to `0004`.
- **`internal/cli/system_mgmt.go`** (`showSystemStatus`) — `string(output)[:30]` on `systemctl show` output panics if the property line is shorter than 30 bytes (e.g. empty/truncated on some distros), crashing the one CLI path meant to report service health. Guarded with a length check, matching the pattern the adjacent logs-output block already used correctly three lines below.
- **`internal/cli/export.go`** — `--format` accepts any string but the write loop only emits rows when `format == "env"`; any other value (including a typo) silently produces a zero-byte file while still printing "Secrets successfully exported ... format at: <path>" and exiting 0. Added upfront validation that rejects anything but `"env"`.
- **`internal/server/rest.go`** (`handleEventWS`) — the client was sent on the unbuffered `Hub.register` channel twice (`client.hub.register <- client` immediately after construction, then `s.Hub.register <- client` — same `*Hub`, same client — a few lines later before starting the pumps). Harmless in steady state, but `Hub.Run` stops servicing `register` the instant its context is cancelled; a shutdown landing between the two sends would permanently block the second send, leaking the HTTP handler goroutine per connection caught in that window. Removed the redundant first send.

**Triaged and deferred, not fixed in this pass:**

- **`internal/cli/sync.go`, `status.go`, `apply.go`'s connectivity check** — the most severe findings of the round. `triggerReconciliation` connects to the agent socket only to confirm it's reachable, then fabricates a `SyncResult{Succeeded: true, ...}` without ever asking the agent to do anything — confirmed by reading `pkg/api/agent_api.go`: the daemon's entire RPC surface (`AgentAPI`) is a single `GetSecret` method, there is no reconciliation-trigger or status-query RPC to call. `status.go`'s `gatherContainers`/`gatherCache`/`gatherRotations`/`gatherQueue` similarly return hardcoded example data with a `Status.Health` computed from it, and `apply.go`'s `verifyProviderConnectivity` only checks that the provider *type* string is one of five known values, never dialing anything. **Not fixed here** because a correct fix means adding new `AgentAPI` RPC methods and server-side handlers backed by the real `StateTracker`/watcher/provider-store state — a feature addition, not a bugfix, and riskier to land without dedicated design attention than the five contained fixes above. Recorded so it isn't lost: these three commands currently present fabricated data as live operational truth to an operator, which is worse than an explicit "not implemented" error.
- **Bootstrap installer (`internal/bootstrap/permissions.go`)** — `os.Chmod`/`os.Chown` follow symlinks; if a pre-existing path DSO expects to own (e.g. `/etc/dso`, `/etc/dso/dso.yaml`) is actually a symlink (leftover from a failed prior install, or planted by another writer of the parent directory), root-privileged chmod/chown silently applies to the symlink's target instead of failing. Fix direction: `os.Lstat` each path first and refuse to proceed if it's a symlink, or route through `FilesystemOps.ValidatePath`, which already does this correctly elsewhere in the same package.
- **Bootstrap rollback (`rollback.go`)** — `WriteConfigOp`/`InstallServiceOp` rollback unconditionally deletes the config/unit file on failure, without checking whether it existed (and had different content) before this run — a failed re-run of bootstrap on an already-configured host can roll back to *no config and no service* rather than the prior working state.
- **Bootstrap lock (`filesystem.go`)** — `BootstrapLockOps.Acquire` never checks liveness of the PID recorded in the lock file, so a lock orphaned by `kill -9`/OOM/power loss wedges every future bootstrap attempt until an operator manually removes it.
- **Setup engine (`internal/setup`)** — six findings: `executor_group.go`'s add-member rollback is dead code (the `After` snapshot never actually reflects added members, so removal on rollback never fires); `executor_file.go`/`executor_directory.go` record their intended mode as the "after" transaction state even though `os.WriteFile`/`os.MkdirAll` don't change mode on an already-existing path, so re-running setup over an over-permissive `dso.yaml`/`/etc/dso` silently fails to tighten it while the transaction log claims success; `owner_unix.go`'s numeric-UID ownership fallback can't be resolved by `apply.go`'s name-based `user.Lookup`, so ownership rollback fails whenever the original owner has no `/etc/passwd` entry.
- **Server/API (`internal/server`)** — the WebSocket endpoint (`/api/events/ws`) is explicitly exempted from per-IP rate limiting and has no concurrent-connection cap, so an authorized caller (or any caller, on a loopback/no-token deployment) can open unbounded connections, each costing a 256-slot channel and two goroutines; the rate limiter's own IP-keyed map has no cap on distinct entries between its 5-minute cleanup cycles; the `severity` query parameter actually filters on the unrelated `status` field (cosmetic naming bug, not a security issue — events never carry a `severity` key at all).
- **Injector/analyzer (`internal/analyzer/container_analyzer.go`)** — `HasContainerName` is set whenever `container.Name != ""`, but Docker's `ContainerInspect` always populates `Name` with an auto-generated value even when no name was explicitly configured, so this is true for essentially every container. It currently masks part of Decision 32's `IsStateful` fix's blast radius (its universal -20 was incidentally helping push many containers below the rolling threshold) rather than measuring what it claims to measure — a real fix needs a way to distinguish an explicitly-configured name from Docker's default, which isn't derivable from `ContainerInspect` output alone. Also: stateful-workload detection only recognizes `mysql`/`postgres`/`mongo` in the image name plus a `/var/lib/` mount prefix, so Redis, Elasticsearch, Cassandra, and any custom-tagged database image are misclassified as safe to scale.
