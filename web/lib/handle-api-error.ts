...
export function handleApiError(error: unknown): never {
  ...
  if (axios.isAxiosError(error)) {
    ...
    switch (status) {
      case 401:
        // No client-visible session token to clear (it's an HttpOnly
        // cookie) -- just send the user back to the login page.
        redirectToLogin()
...
/**
 * Redirects to /login -- unless we're already there, to avoid a redirect
 * loop when an unauthenticated request fires from the login page itself.
 * There are no client-visible auth tokens to clear (the session lives in an
 * HttpOnly cookie the server clears on logout / expiry).
 */
