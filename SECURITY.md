...
### Provider Plugin Hash Verification [SEC-2]

External provider plugin binaries (AWS, Azure, Vault, Huawei) are verified
against a SHA256 hash manifest before being executed. Verification is
**mandatory by default (fail closed)**: a missing manifest, a plugin absent
from the manifest, or a hash mismatch all reject the plugin load.

- **Default manifest location**: `<plugin_dir>/hashes.txt`, auto-generated by
  `docker dso system setup --providers <name>` — no manual configuration
  needed for a standard install
- **`DSO_PLUGIN_HASH_MANIFEST`**: overrides the manifest path for
  non-standard layouts; it is a path override, not an on/off switch
- **`DSO_PLUGIN_SKIP_HASH_VERIFY=1`**: the sole, explicit escape hatch for
  bring-up/emergency use — always logs a loud warning when used
- **Known limitation**: this closes the "silently accept a substituted
  binary" gap, but does not close the underlying check-then-execute window —
  a small window still exists between hashing the binary and `exec`-ing it,
  during which a file could theoretically be swapped by an attacker who
  already has write access to the plugin directory. This is an inherent
  property of file-path-based verification (not unique to DSO) and was not
  introduced or worsened by this fix; closing it fully would require
