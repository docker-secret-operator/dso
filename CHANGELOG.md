...
- **[SEC-1.1]** Redaction is substantially cheaper per log call. `RedactString` ran all 11 patterns through `ReplaceAllString` unconditionally, and that call allocates a fresh string **even when the pattern does not match** — the common case. Guarding each with a non-allocating `MatchString` cut `Redaction_StringField` from 24,274 ns/op and **70 allocs** to ~13,600 ns/op and **4 allocs** (~44% faster, 94% fewer allocations); `Redaction_ReflectField` from 47,840 ns/op / 90 allocs to ~28,400 ns/op / 24 allocs.
  - The guard is **provably** equivalence-preserving, not heuristic: when `MatchString` is false, `ReplaceAllString` returns the input unchanged. The backlog's "combined regex" and "keyword pre-scan" options were both rejected as risking redacting *less*. Backed by differential tests requiring byte-identical output against the original implementation across a hand-picked matrix and 2,197 fuzzy combinations; all 37 acceptance-criteria subtests pass unchanged.
...
- **[REL-7] Server-side deadline propagation** completes REL-6. The daemon now sends its deadline to the plugin so a context-aware provider can bound its own SDK call, instead of continuing to burn a cloud API request the daemon has already abandoned. Added **additively** as a new `Plugin.GetSecretWithDeadline` RPC method with transparent fallback, rather than widening `GetSecret`'s gob argument — so already-installed plugin binaries keep working with no rebuild and no wire-format change. All four plugins now implement `SecretProviderWithContext`; AWS/Azure/Vault genuinely honor the deadline, Huawei performs a pre-flight check only (its SDK takes no context — documented limitation).
- **[PROV-1] All six providers now deliver an initial value immediately from `WatchSecret`.** AWS/Azure/Huawei did; Vault, `file` and `env` only emitted on their first tick, so a consumer waiting for an initial value stalled a full interval on half the providers.
- **[PROV-2] AWS resource tags can no longer shadow secret data.** Tags were merged into the secret's own key namespace as `_TAG_<key>` with a plain assignment. Tags are typically writable by a broader IAM population than `secretsmanager:GetSecretValue` readers, so a tag named `password` (or literally `_TAG_password`) could silently overwrite real secret material. Secret data now wins and the collision is reported.
- **[TEST-1] Converted warning-only ("toothless") tests into real assertions.** A sweep of 127 test files found 13 instances that computed an expected-vs-actual comparison but only called `t.Logf`, so they could never fail a build. Notably **none turned out to be a production defect** — each was a wrong expectation or a broken fixture. Highlights: `TestStress_EventDebouncer_RapidFire`'s generator produced only 2 duplicate events instead of 8,000, so the debouncer was never actually exercised (now 7,992); `TestEventReactorImpl_QueueMaxBatch` waited 12s for work that needs three 5s ticks; and `TestIsSafePathEmptyPaths` asserted errors that `IsSafePath`'s deliberate "anywhere mode" does not produce (test corrected, **production code unchanged**). Timing/resource heuristics were deliberately left as logs to avoid flaky CI.
...
- **[SEC-7]** Added opt-in `proxy.allowed_host_ports` config (exact ports or ranges, e.g. `["8080", "3000-4000"]`) to restrict which host ports the proxy may bind for containers declaring `dso.host_ports`. Unset preserves the existing "allow any port >= 1024" behavior.

### Fixed

- **[SEC-6]** Fixed shell command injection in `internal/injector/inject.go`: a secret file name derived from an untrusted compose service name could contain shell metacharacters and break out of the `docker exec` command. Now validated against an allow-list.
- **[REL-1]** Fixed rotation state being marked "complete" in `internal/watcher/controller.go`/`internal/agent/trigger.go` before the async rotation work (rolling/restart/compose) had actually finished. `TriggerReload` now takes an `onComplete` callback fired only once the real outcome is known.
- **[SEC-7]** Rejected privileged (<1024) and out-of-range host ports in `ParseHostPorts`, closing the main gap ahead of the full allow-list above.
- **[REL-2]** Fixed a data race in `internal/providers/store.go`: `GetProvider` read `LastHealthy` without the mutex that guards it.
- **[REL-3]** Fixed unbounded growth of the event-dedup `lastSeen` map (`internal/events/event_reactor_impl.go`) with a periodic eviction sweep.
- **[REL-4]** Fixed unbounded growth of `LockManager.locks` (`internal/rotation/lock_manager.go`); entries are now reference-counted and removed once nothing holds or waits on them.
- **[REL-5]** Fixed `ContainerListener.Stop()` (`internal/events/container_listener.go`) leaving the listener unable to restart, and a redundant blocking send that stalled every `Stop()` call by up to 100ms.
- **[QUAL-1]** Fixed a swallowed JSON unmarshal error in `internal/cli/logs.go`'s `fetchEvents` that made a malformed API response look identical to "no events".
- **[QUAL-2]** Fixed a swallowed `filepath.Abs` error in `RunComposeUpWithEnv` (`internal/core/compose.go`) that could corrupt the `dso.compose.path` label.
- **[QUAL-3]** Fixed a swallowed `yaml.Marshal` error in `PrintRedactedCompose` (`internal/core/compose.go`) that silently produced no `--debug` output.
- **[BUG-1]** Fixed a concurrent-map-write panic: the `tickers` map in `runMainLoop()` was accessed from multiple goroutines without synchronization.
- **[BUG-4]** Fixed `internal/proxy.Manager.SwapBackend`'s deferred-removal goroutine outliving `Manager.Stop`; it's now bound to the Manager's own context/`WaitGroup`.
- **[AUDIT-1]** Wired the existing, unit-tested `internal/audit` compliance logger into `AgentServer.GetSecret`, the single RPC handler every secret fetch passes through — it previously had zero production callers.
- **[AUDIT-2]** Audit log now records the real, authenticated peer identity (via `SO_PEERCRED` → `os/user.LookupId`) instead of a fixed `"agent"` placeholder.
- **[AUDIT-3]** Extended the audit trail to rolling (blue-green) rotation outcomes, not just secret access.
- **[CLEAN-3]** Excluded vendored `web/node_modules` from `golangci-lint`/`gosec` so an npm package's bundled Go code no longer produces false findings on toolchain bumps.

### Removed

- **[CLEAN-1]** Removed confirmed-dead code: `internal/runtime` (test-only, zero importers) and `internal/providers/plugin_verifier.go` (superseded by SEC-2's `enforceHashVerification`).
- Removed an untracked, gitignored `internal/webui` build artifact from the working tree — unrelated to the real web dashboard on `feature/web-ui`.

### Security

- **[SEC-1]** Wired the existing `pkg/security` redaction engine into all production logging paths, closing the gap where the documented "no secrets in logs" guarantee (`SECURITY.md`) was unimplemented. Covers all zap field types (`String`, `Error`, `Any`/`Reflect`, `Binary`, arrays) while preserving sampling, level filtering, and secret-identifier fields like `secret_name`.
- **[SEC-2]** Made provider plugin hash verification mandatory by default (previously silently disabled unless `DSO_PLUGIN_HASH_MANIFEST` was set). Installer and Dockerfile now auto-generate the manifest; an explicit `DSO_PLUGIN_SKIP_HASH_VERIFY=1` escape hatch exists for bring-up.
- **[SEC-3]** Removed daemon write-ownership of the container plugin directory, so a compromised daemon process can no longer substitute a plugin binary and regenerate SEC-2's hash manifest to match.
- **[SEC-4]** `pkg/config.IsSafePath` now resolves symlinks before trusting a path is contained, closing a previously-observed-but-unenforced symlink-escape gap.
- **[SEC-5]** Made CI's `gosec`/`govulncheck` scans blocking (previously advisory-only); resolved 19 of 22 reachable vulnerabilities via a toolchain/dependency bump and triaged all pre-existing `gosec` findings. Enabled SBOM generation on release builds.

### Testing

- **[QUALITY-1]** Raised `internal/proxy` test coverage from 5.9% to 86.6%, covering the TCP reverse-proxy's routing core (`registry.go`, `router.go`) which previously had none.
