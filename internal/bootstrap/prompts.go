...
func (p *InteractivePrompter) PromptBootstrapMode() (BootstrapMode, error) {
	...
	_, _ = fmt.Fprintf(p.stdout, "===========================\n\n")
	_, _ = fmt.Fprintf(p.stdout, "1. Local   - For local development/testing (stores config locally, no systemd)\n")
	_, _ = fmt.Fprintf(p.stdout, "2. Agent   - For production cloud deployment (systemd managed, auto-rotate)\n\n")
...
func (p *InteractivePrompter) PromptCloudProviderConfirmation(detected *CloudProviderInfo) (bool, error) {
	...
	_, _ = fmt.Fprintf(p.stdout, "========================\n\n")
	_, _ = fmt.Fprintf(p.stdout, "Detected: %s\n\n", detected.Provider)
...
func (p *InteractivePrompter) PromptProviderSelection() (string, error) {
	...
	_, _ = fmt.Fprintf(p.stdout, "========================\n\n")
	_, _ = fmt.Fprintf(p.stdout, "1. AWS     - Amazon Web Services (EC2 IAM roles)\n")
	_, _ = fmt.Fprintf(p.stdout, "2. Azure   - Microsoft Azure (Key Vault)\n")
	_, _ = fmt.Fprintf(p.stdout, "3. Huawei  - Huawei Cloud (KMS)\n")
	_, _ = fmt.Fprintf(p.stdout, "4. Vault   - HashiCorp Vault (self-hosted)\n")
	_, _ = fmt.Fprintf(p.stdout, "5. None    - No cloud provider (local secrets)\n\n")
...
func (p *InteractivePrompter) PromptAWSRegion() (string, error) {
	_, _ = fmt.Fprintf(p.stdout, "\nAWS Configuration\n")
	_, _ = fmt.Fprintf(p.stdout, "=================\n\n")
	_, _ = fmt.Fprintf(p.stdout, "Common regions: us-east-1, us-west-2, eu-west-1, ap-southeast-1\n\n")
...
func (p *InteractivePrompter) PromptAzureVaultURL() (string, error) {
	_, _ = fmt.Fprintf(p.stdout, "\nAzure Configuration\n")
	_, _ = fmt.Fprintf(p.stdout, "===================\n\n")
	_, _ = fmt.Fprintf(p.stdout, "Example: https://my-vault.vault.azure.net/\n\n")
	...
	for {
		...
		if err := p.validateURL(urlStr); err != nil {
			_, _ = fmt.Fprintf(p.stdout, "Error: %v. Please try again.\n", err)
			continue
		}

		// Ensure it's HTTPS for Azure Key Vault
		if !strings.HasPrefix(urlStr, "https://") && !strings.HasPrefix(urlStr, "http://") {
			_, _ = fmt.Fprintf(p.stdout, "Error: URL must start with http:// or https://. Please try again.\n")
			continue
		}

		// Check it looks like an Azure vault URL
		if !strings.Contains(urlStr, "vault.azure.net") {
			_, _ = fmt.Fprintf(p.stdout, "Warning: URL doesn't look like an Azure Key Vault URL (should contain 'vault.azure.net')\n")
		}
...
func (p *InteractivePrompter) PromptHuaweiRegionAndProject() (region, projectID string, err error) {
	...
	_, _ = fmt.Fprintf(p.stdout, "==========================\n\n")
	_, _ = fmt.Fprintf(p.stdout, "Common regions: ap-southeast-1, ap-southeast-2, cn-north-1\n\n")
...
func (p *InteractivePrompter) PromptVaultAddress() (string, error) {
	...
	_, _ = fmt.Fprintf(p.stdout, "==============================\n\n")
	_, _ = fmt.Fprintf(p.stdout, "Example: http://vault.internal:8200\n\n")
	...
	for {
		...
		if !strings.HasPrefix(address, "http://") && !strings.HasPrefix(address, "https://") {
			_, _ = fmt.Fprintf(p.stdout, "Error: Address must start with http:// or https://. Please try again.\n")
			continue
		}
...
func (p *InteractivePrompter) PromptSecrets(provider string) ([]SecretDefinition, error) {
	...
	for {
		...
		if seenSecrets[secretName] {
			_, _ = fmt.Fprintf(p.stdout, "Error: Secret '%s' already defined. Please use a different name.\n", secretName)
			continue
		}
		...
		switch provider {
		case ProviderAWS:
			_, _ = fmt.Fprintf(p.stdout, "Example: arn:aws:secretsmanager:region:account:secret:name\n")
		case ProviderAzure:
			_, _ = fmt.Fprintf(p.stdout, "Example: secret-name (without vault URL)\n")
		case ProviderHuawei:
			_, _ = fmt.Fprintf(p.stdout, "Example: kms-secret-name\n")
		case ProviderVault:
			_, _ = fmt.Fprintf(p.stdout, "Example: path/to/secret\n")
		}
		...
		for {
			...
			if len(parts) != 2 {
				_, _ = fmt.Fprintf(p.stdout, "Invalid format. Use: SECRET_KEY=ENV_VAR\n")
				continue
			}
			...
			if secretKey == "" || envVar == "" {
				_, _ = fmt.Fprintf(p.stdout, "Key and value cannot be empty\n")
				continue
			}
		...
		if len(mappings) == 0 {
			_, _ = fmt.Fprintf(p.stdout, "Secret must have at least one mapping, skipping\n")
			continue
		}
		...
		_, _ = fmt.Fprintf(p.stdout, "Secret '%s' added with %d mappings\n\n", secretName, len(mappings))
	}

	if len(secrets) == 0 {
		_, _ = fmt.Fprintf(p.stdout, "Warning: No secrets defined\n")
	}
...
func (p *InteractivePrompter) PromptConfigPath() (string, error) {
	...
	_, _ = fmt.Fprintf(p.stdout, "=======================\n\n")
	_, _ = fmt.Fprintf(p.stdout, "Default: /etc/dso/dso.yaml\n\n")
...
func (p *InteractivePrompter) DisplaySummary(mode BootstrapMode, provider string, config *Config) error {
	...
	_, _ = fmt.Fprintf(p.stdout, "Mode:     %s\n", mode)
	_, _ = fmt.Fprintf(p.stdout, "Provider: %s\n", provider)
	_, _ = fmt.Fprintf(p.stdout, "Providers configured: %d\n", len(config.Providers))
	_, _ = fmt.Fprintf(p.stdout, "Secrets configured:   %d\n", len(config.Secrets))
	...
	if len(config.Secrets) > 0 {
		...
		for _, secret := range config.Secrets {
			_, _ = fmt.Fprintf(p.stdout, "  - %s (provider: %s, mappings: %d)\n",
...
func (p *InteractivePrompter) DisplayCompletionMessage(configPath string, mode BootstrapMode) error {
	...
	_, _ = fmt.Fprintf(p.stdout, "Configuration written to: %s\n", configPath)

	if mode == ModeAgent {
		_, _ = fmt.Fprintf(p.stdout, "\nNext steps:\n")
		_, _ = fmt.Fprintf(p.stdout, "1. Enable DSO agent: sudo systemctl enable dso-agent\n")
		_, _ = fmt.Fprintf(p.stdout, "2. Start DSO agent:  sudo systemctl start dso-agent\n")
		_, _ = fmt.Fprintf(p.stdout, "3. Check status:     sudo systemctl status dso-agent\n")
		_, _ = fmt.Fprintf(p.stdout, "4. View logs:        sudo journalctl -u dso-agent -f\n")

		_, _ = fmt.Fprintf(p.stdout, "\nFor non-root CLI usage, run:\n")
		_, _ = fmt.Fprintf(p.stdout, "1. sudo usermod -aG dso $USER\n")
		_, _ = fmt.Fprintf(p.stdout, "2. sudo usermod -aG docker $USER\n")
		_, _ = fmt.Fprintf(p.stdout, "3. Log out and back in\n")
	} else {
...
func (p *InteractivePrompter) promptInput(prompt string) (string, error) {
	_, _ = fmt.Fprintf(p.stdout, "%s: ", prompt)
