...
func TestSecurityValidation(t *testing.T) {
	...
	users.Create(ctx, &storage.User{ID: "u-viewer", Username: "viewer", Role: "viewer"})
	users.Create(ctx, &storage.User{ID: "u-operator", Username: "operator", Role: "operator"})
	users.Create(ctx, &storage.User{ID: "u-reviewer", Username: "reviewer", Role: "reviewer"})
	users.Create(ctx, &storage.User{ID: "u-approver", Username: "approver", Role: "approver"})
	users.Create(ctx, &storage.User{ID: "u-admin", Username: "admin2", Role: "admin"})
	users.Create(ctx, &storage.User{ID: "u-disabled", Username: "disabled", Role: "viewer", Disabled: true})
	...
	login := func(userID string) string {
		user, err := provider.Users().GetByID(ctx, userID)
		if err != nil || user == nil {
			return ""
		}
